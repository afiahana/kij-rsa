...
def read_msg(sock):
    while True:
        ...
        if len(data.split(',')) == 3:
            ...
            print("\n\n-----Receiving Message-----")
            print("\nMessage before decrypted: " + DES.bin2hex(msg)) # Message sebelum di-decrypt
            msg_key = rsa.decrypt(d, N, enc_key)
            key_rev = msg_key.split()[::-1]
            print("After decrypted:")
            msg = DES.encrypt(msg, key_rev) # Decrypt Message
            print("(" + user + ")" + " : " + DES.bin2ascii(msg)) # Ubah message menjadi string
...
while True:
    ...
    if dest+'.txt' not in key_folder:
        ...
        if len(msg) != 8:
            ...
            cipher_text = DES.encrypt(msg, rkb) # Encrypt message dengan DES
            print("Encrypted message: " + DES.bin2hex(cipher_text)) # Message setelah di-encrypt
